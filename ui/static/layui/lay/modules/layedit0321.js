...
layui.define(['layer', 'form', 'code'], function (exports) {
  ...
  var setIframe = function (editor, textArea, set) {
    ...
    , insertInline = function (tagName, attr, range) {
      console.log(tagName, attr, range)
    ...
    , toolActive = function (iframeWin, editor, set) {
      var iframeDOM = iframeWin.document
        ...
        // 插入音乐
        , music: function (range) {
          console.log(range)
          var that = this;
          layer.open({
            type: 1
            , id: 'fly-jie-music-upload'
            , title: '音乐'
            , shade: 0.05
            , shadeClose: true
            , area: function () {
            ...
            , content: ['<ul class="layui-form layui-form-pane" style="margin: 20px 20px 0 20px">'
              ...
              , '<label class="layui-form-label" style="width: 110px;position: relative;z-index: 10;">名称</label>'
              , '<input type="text" required name="nameStr" placeholder="请输入名称-作者" style="position: absolute;width: 100%;padding-left: 120px;left: 0;top:0" value="" class="layui-input">'
              , '</li>'
              , '<li class="layui-form-item" style="position: relative">'
              , '<button type="button" class="layui-btn" id="LayEdit_InsertMusic" style="width: 110px;position: relative;z-index: 10;"><i class="layui-icon"></i>上传音乐</button>'
              , '<input type="text" name="musicSrc" placeholder="请选择文件" style="position: absolute;width: 100%;padding-left: 120px;left: 0;top:0" class="layui-input">'
              , '</li>'
              , '</ul>'].join('')
            , btn: ['确定', '取消']
            , btnAlign: 'c'
            , yes: function (index, layero) {
              var styleStr = "", nameStr = layero.find('input[name="nameStr"]'),
                musicSrc = layero.find('input[name="musicSrc"]');
              ...
              if (musicSrc.val() == '') {
                ...
                if (nameStr.val() != "") attrs["alt"] = nameStr.val();
                if (styleStr != "") attrs["style"] = styleStr;
                insertInline.call(iframeWin, 'audio', attrs, range);
                layer.close(index);
              }
            }
            , btn2: function (index, layero) {
              var callDel = set.calldel;
              if (callDel.url != "") {
                $.post(callDel.url, {"musicPath": layero.find('input[name="musicSrc"]').val()}, function (res) {
                  callDel.done(res);
                })
              }
            }
            ...
            , success: function (layero, index) {
              layui.use('upload', function (upload) {
                ...
                var loding, nameStr = layero.find('input[name="nameStr"]'),
                  musicSrc = layero.find('input[name="musicSrc"]');
                var uploadImage = set.uploadImage || {};
                if (uploadImage.url == "") {
                  layer.msg("上传接口配置错误！");
                ...
                upload.render({
                  ...
                  , done: function (res, input, upload) {
                    ...
                    if (res.code == 0) {
                      res.data = res.data || {};
                      musicSrc.val(res.data.src);
                      nameStr.val(res.data.name);
    ...
    , tools = {
      ...
      ,
      music: '<i class="layui-icon layui-icon-headset" title="音乐" layedit-event="music" style="font-size:18px"></i>'
      ,
